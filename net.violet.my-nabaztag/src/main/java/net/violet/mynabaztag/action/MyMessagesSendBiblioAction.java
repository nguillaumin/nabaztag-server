...
import net.violet.mynabaztag.form.MyMessagesSendBiblioForm;
...
public class MyMessagesSendBiblioAction extends ActionWithLog {
	@Override
	protected ActionForward doExecute(ActionMapping mapping, ActionForm form, HttpServletRequest request, @SuppressWarnings("unused") HttpServletResponse response) {
		...
		final MyMessagesSendBiblioForm myForm = (MyMessagesSendBiblioForm) form;
		...
		int idBiblio = myForm.getIdBiblio();

		final String param = SessionTools.getSessionUserpref(session, "page");
		final String biblio = request.getParameter("idBiblio");
		if (!(param.equals(StringShop.EMPTY_STRING)) && param.equals("home") && (biblio != null)) {
			idBiblio = Integer.parseInt(biblio);
		}

		String destName = myForm.getDestName();
		...
		if (myForm.getSend() == 1) {
			...
			if (objectDest == null) {
				...
				final Music theMusic = Factories.MUSIC.find(idBiblio);
				if ((theMusic == null) || (theMusic.getFile() == null)) {
					message_erreur = "server_error"; // problème le son n'existe pas
				} else {
			...
			final Music theMusic = Factories.MUSIC.find(idBiblio);
			if ((theMusic != null)) {
				final MusicData theMusicData = MusicData.getData(theMusic);
				url = theMusicData.getListenPath();
			}
			myForm.setUrl(url);
