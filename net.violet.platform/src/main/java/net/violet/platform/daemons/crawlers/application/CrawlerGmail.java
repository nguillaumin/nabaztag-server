...
public class CrawlerGmail extends AbstractSubscriptionCrawler<TTSProcessUnit> {
	...
	private static final String URL = ApplicationSettingData.findByApplicationAndKey(ApplicationData.getData(CrawlerGmail.APPLICATION), ApplicationSetting.FeedHandler.URL).getValue();

	private final FeedCrawlerSender<TTSProcessUnit> mSender;
	private final FeedCrawlerProcessor<TTSProcessUnit> mProcessor;

	public CrawlerGmail(String[] inArgs) {
		super(inArgs, true);
	...
	public Service getService() {
		return ServiceFactory.SERVICE.GMAIL.getService();
	}

	@Override
	protected int getNbItems2Read() {
		return CrawlerGmail.NB_NEWS_2_READ;
	}
	...
	public void processNews(List<Files> inNewsFiles, List<TTSProcessUnit> inNews, MessageSignature inMessageSignature, String inTitle, FeedUnit inFeedUnit) {
		...
		if (inFeedUnit instanceof AbstractSubscriptionCrawler.SubscriptionUnit) {
			...
			if ((theAnnounceSetting == null) || theAnnounceSetting.toString().equals(net.violet.common.StringShop.EMPTY_STRING)) {
				fileId = CrawlerGmail.APPLICATION.getProfile().getAnnounceFile().getId();
			} else {
	...
	protected long getApplicationId() {
		return CrawlerGmail.APPLICATION.getId();
	}

	@Override
	protected String getUrl() {
		return CrawlerGmail.URL;
	}
}
