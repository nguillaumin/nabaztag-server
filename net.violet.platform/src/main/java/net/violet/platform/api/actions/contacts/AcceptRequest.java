...
public class AcceptRequest extends AbstractContactAction {

	@Override
	protected Object doProcessRequest(ActionParam inParam) throws ForbiddenException, InvalidSessionException, InvalidParameterException, NoSuchContactException, InvalidContactException, ContactAlreadyExistsException {
		...
		if (theSessionUser.equals(theContact.getContact())) {
			...
			if (theContact.getStatus().equalsIgnoreCase(Contact.STATUS.PENDING.toString())) {
				...
				final boolean add = inParam.getBoolean("add_sender", Boolean.FALSE);

				if (add) { // le contact veux aussi etre l'ami du demandeur
					Add.addContact(theSessionUser, theContact.getPerson(), inParam.getCaller());
				}
				theContact.acceptContact();
				result = true;
			}
