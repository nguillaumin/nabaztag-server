...
public class JabberComponentObjectsPacketListener extends AbstractPacketListener {

	private static final Logger LOGGER = Logger.getLogger(JabberComponentObjectsPacketListener.class);
	...
	private static final int NB_THREADS = Constantes.NB_COMPOSANT_OBJECT_THREADS;

	public JabberComponentObjectsPacketListener(JabberComponent inComponent) {
		super(JabberComponentObjectsPacketListener.NB_THREADS);
		this.mComponent = inComponent;
	}

	@Override
	protected void doProcessPacket(Packet inPacket) {
		JabberComponentObjectsPacketListener.LOGGER.debug("Into doProcessPacket : " + inPacket.toXML());
		if (inPacket instanceof IQSources) {
			processIQSourcesPacket((IQSources) inPacket);
			JabberComponentObjectsPacketListener.LOGGER.debug("PROCESS " + inPacket.toXML());
		} else if (inPacket instanceof PingPongPacketTest) {
			JabberComponentObjectsPacketListener.LOGGER.debug("Process PingPongPacket");
	...
	private void processIQSourcesPacket(IQSources inPacket) {
		if (inPacket.getType() == Type.GET) {
			...
			if (theObject != null) {
				...
				JabberComponentObjectsPacketListener.LOGGER.info("XMPP OBJECT BOOT " + theObject.getObject_login() + " (" + theObject.getObject_serial() + ")");
				...
				JabberComponentObjectsPacketListener.LOGGER.info("IQ SOURCES: OBJET INCONNU, FROM = " + inPacket.getFrom());
	...
	private void processEarsExtension(Message inMessage, EarsPacket inExtension) {
		...
		if (theFrom != null) {
			...
			if ((leftEarPosObj != null) || (rightEarPosObj != null)) {
				...
				// -> communion d'oreille.
				final VObject theObject = Factories.VOBJECT.findByJID(theFrom);
				if (theObject != null) {
					theObject.setLastActivityTime();
					int earl;
					int earr;
