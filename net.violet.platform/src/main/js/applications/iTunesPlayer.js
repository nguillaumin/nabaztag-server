...
appDescr = {
	apiKey: "iTunesPlayer",
	name: "net.violet.js.itunesplayer"
};
...
appEventHandlers = {
	...
	onTriggerEvent: function(objectId, event) {
		...
		var decodeAction = {"play": "Playing ", "toggle": "Pause/resume ", "previous": "Previous track ", "next": "Next track ", "stop": "Quit iTunes "};
		var resume = decodeAction[action];

		var playOptions = {};
		if (settings.artistname) {
			playOptions.artistname = settings.artistname;
			resume += (settings.artistname + " ");
		}
		if (settings.albumname)  {
			playOptions.albumname  = settings.albumname;
			resume += ("album " + settings.albumname + " ");
		}
		if (settings.trackname)  {
			playOptions.trackname  = settings.trackname;
			resume += ("track " + settings.trackname + " ");
		}
		if (settings.playlist)   {
			playOptions.playlist   = settings.playlist;
			resume += ("playlist " + settings.playlist + " ");
		}
