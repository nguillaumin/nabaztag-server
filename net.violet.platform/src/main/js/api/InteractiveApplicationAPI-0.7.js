...
InteractiveApplication = function(appDescr, eventMap) {
	...
	for (var attr in eventMap) {
		...
		if (attr && attr.startsWith("on") && (typeof eventMap[attr] == "function")) {
			...
			debug("Added event handler " + attr);
		} else {
...
InteractiveApplication.prototype = {
	...
	getEventHandler: function(evtName) {
		...
		debug("looking for event handler " + evtName + " in " + this.eventHandlers);
	...
	processEvent: function(event) {
		...
		debug(this + " PROCESSING EVENT " + evtName + " FOR OBJECT " + object + " in context : " + ctx);
		var objectId = object.id;

		// get the correct event handler..
		var eventHnd = this.getEventHandler("on" + evtName + "Event");

		try {
			// ..handle it !
			var eventResponse = eventHnd.apply(this, [objectId, event]);
			debug(" EVENT HANDLER RESPONSE : " + eventResponse);
			...
			return [violet.objects.createMessage(objectId)
				...
				.alternativelyAddTextToSpeech(error, "en", "US-Liberty")
				.stopInteractive() // the platform will have to propagate this to all involved objects
				.serialize()];
		}
	},
...
//----------------------------------------------------------------------------80
	...
	Settings = function(map) {
		return (map.settings) ? Object.extend(this, map.settings) : Object.extend(this, map); // copy the map content
	};
	...
	Object.extend(Settings.prototype, {
		_buildOption: function(inOptions) {
			...
			if ((optionType == 'string') || (optionType == 'date')) return {defaultValue: inOptions};
			return inOptions;
		},
...
violet.util.namespace("violet.objects", {
	...
	createMessage: function(to) {
		debug("Creating new message for " + to);
...
violet.objects.message.prototype = {
	...
	/**
	 * Take a list of objects to add as destinatairs
	forBoyard: function() {
		for (var obj, i = 0; obj = arguments[i]; i++) {
...
(function() {
	...
	var messageMethods = {
		...
		playAudioStream: function(inUrl, inOptions) {
			var sndMap = {type: "expression", modality: "net.violet.sound.mp3", url: inUrl, streaming: true, withEar: true};
			if (inOptions) {
