...
public class AcceptRequestTest extends AbstractTestBase {

	@Test
	public void testAccept() throws APIException {
		final Date now = new Date();

		final User theUser1 = new UserMock(1, "blabla", "12345", "test1@violet.net", getFrLang(), "France", "firstName", "lastName", getParisTimezone(), Annu.MALE, "75011", "Paris", 0);
		...
		final Contact theContact = new ContactMock(5, theUser1, getPrivateUser(), Contact.STATUS.PENDING);
		new ContactMock(6, theUser1, theUser3, Contact.STATUS.REJECTED);

		final Action theAction = new AcceptRequest();
		...
		final Boolean theResultAsBool = (Boolean) theResult;
		Assert.assertTrue(theResultAsBool);

		Assert.assertTrue(ContactData.getContactByUserAndContact(UserData.getData(theUser1), UserData.getData(getPrivateUser())).isValid());

		final ContactData theContactData = ContactData.getContactByUserAndContact(UserData.getData(getPrivateUser()), UserData.getData(theUser1));

		Assert.assertFalse(theContactData.isValid());

	}

	@Test
	public void testDoubleAccept() throws APIException {
		final Date now = new Date();
		...
		theParams.put(ActionParam.SESSION_PARAM_KEY, SessionManager.generateSessionId(caller, UserData.getData(getPrivateUser()), theCalendar.getTime()));
		theParams.put("add_sender", true);

		final ActionParam theActionParam = new ActionParam(caller, theParams);

		final Object theResult = new AcceptRequest().processRequest(theActionParam);

		Assert.assertNotNull(theResult);
		Assert.assertTrue(theResult instanceof Boolean);
		Assert.assertTrue((Boolean) theResult);

		final ContactData contact = ContactData.getContactByUserAndContact(UserData.getData(theUser1), UserData.getData(getPrivateUser()));
		Assert.assertTrue(contact.isValid());
		Assert.assertEquals(Contact.STATUS.ACCEPTED.toString().toLowerCase(), contact.getStatus());

		final ContactData theContactData = ContactData.getContactByUserAndContact(UserData.getData(getPrivateUser()), UserData.getData(theUser1));
		Assert.assertTrue(theContactData.isValid());
		Assert.assertEquals(Contact.STATUS.AUTOMATICALLY_ACCEPTED.toString().toLowerCase(), theContactData.getStatus());
	}

	@Test
	public void testAcceptLater() throws APIException {
		final Date now = new Date();
		...
		new ContactMock(1, getPrivateUser(), getKowalskyUser(), Contact.STATUS.ACCEPTED);
		final Contact theContact = new ContactMock(3, theUser2, getPrivateUser(), Contact.STATUS.REJECTED);
		new ContactMock(4, getPrivateUser(), theUser3, Contact.STATUS.REJECTED);
	...
	public void testAcceptInvalidContact() throws APIException {
		final Date now = new Date();
		...
		final Contact theContact = new ContactMock(5, theUser1, theUser2, Contact.STATUS.PENDING);
