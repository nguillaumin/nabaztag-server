...
public class GetTest extends AbstractTestBase {
	...
	public void testFind() throws APIException {
		...
		final String userId = UserData.getData(getPrivateUser()).getApiId(caller);
		...
		Assert.assertEquals(contactOne.get("email"), null);
		Assert.assertEquals(contactOne.get("id"), UserData.getData(theUser1).getApiId(caller));
		Assert.assertEquals(contactTwo.get("email"), null);
		Assert.assertEquals(contactTwo.get("id"), UserData.getData(getKowalskyUser()).getApiId(caller));
