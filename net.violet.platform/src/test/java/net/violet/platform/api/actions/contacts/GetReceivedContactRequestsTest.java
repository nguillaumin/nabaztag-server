...
public class GetReceivedContactRequestsTest extends AbstractTestBase {
	...
	public void testFind() throws APIException {
		...
		new ContactMock(2, getPrivateUser(), theUser1, Contact.STATUS.AUTOMATICALLY_ACCEPTED);
		...
		final Action theAction = new GetReceivedContactRequests();
		...
		Assert.assertEquals(contactOne.get("person_id"), UserData.getData(theUser4).getApiId(caller));
		Assert.assertEquals(contactOne.get("contact_id"), apiID);
		Assert.assertEquals(contactTwo.get("person_id"), UserData.getData(theUser3).getApiId(caller));
		Assert.assertEquals(contactTwo.get("contact_id"), apiID);
