...
public class RetractTest extends AbstractTestBase {

	@Test
	public void retract() throws APIException {
		...
		Assert.assertTrue(NotificationData.getReceived(VObjectData.getData(theObjectSender), 0, 0).isEmpty());
		...
		Assert.assertEquals(0, Factories.SUBSCRIPTION.findByApplicationAndObject(theEarApplication, theObjectSender).size());
		Assert.assertEquals(0, Factories.SUBSCRIPTION.findByApplicationAndObject(theEarApplication, theObjectRecipient).size());
